...
# zip으로 키들의 배열과 value들의 배열을 묶어서 새로운 딕셔너리를 만들 수 있습니다. - 이해만 일단
# key와 value들의 숫자를 맞추는 것부터 잘 해야한다.
print("=== zip ===")
...
my_classroom = {
    ...
    "반": 1,
    "반장": "홍길동",
    "학생들": ["김철수", "이영희"],  # 리스트도 값이 될 수 있다
}

my_school = [
    {"학년": 3, "반": 1, "반장": "홍길동", "학생들": ["김철수", "이영희"]},
    {"학년": 3, "반": 2, "반장": "고라파덕", "학생들": ["박영희", "이민호"]},
]

# 딕셔너리 안에 value로 딕셔너리를 사용하기
print("=== 딕셔너리 안에 value로 딕셔너리를 사용하기 ===")
kbo = [
    {
        "구단명": "삼성",
        "마스코트": "라이온스",
        "구장": {"1구장": "대구삼성라이온즈파크", "2구장": "포항야구장"},
    },
    {
        "구단명": "두산",
        "마스코트": "베어스",
...
print(kbo[0]["구장"]["1구장"])  # 대구삼성라이온즈파크


# 간단하게 딕셔너리 예제를 만들어봅시다.
# 보통 리스트안에 딕셔너리들이 있다면 그 딕셔너리들은 같은 key들을 갖는 게 일반적
print("=== 복습 ===")

location_dict = {
    "시": [
        {"이름": "서울특별시", "기초단체": ["종로구", "중구", "마포구"]},
        {"이름": "대구광역시", "기초단체": ["수성구", "중구", "달서구"]},
    ],
    "도": [
...
# 위 코드를 보면 두 개의 for문이 사실상 같은 일을 한다. 그래서 중복되는 부분을 묶고
# 다른 점만 외부에서 지적해 시키면 돌아가는 함수를 만들면 효율성이 높아진다.
